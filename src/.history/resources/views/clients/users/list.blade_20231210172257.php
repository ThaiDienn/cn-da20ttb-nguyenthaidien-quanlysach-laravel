...
@endif
<h1>{{$title}}</h1>
<a href="{{route('admin.add')}}" class="btn btn-primary">Thêm đọc giả</a>
<hr>
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="{{ route('admin.edit', ['id'=>$item->id] ) }}" class="btn btn-warning btn-sm">Sửa</a>
            </td>
            <td>
                <a onclick="return confirm('Bạn có chắc chắn muốn xóa?')" href="{{ route('admin.delete', ['id'=>$item->id] ) }}" class="btn btn-danger btn-sm">Xóa</a>
            </td>
        </tr>
            @endforeach
        @else
        <tr>
            <td colspan="6" class="center">Không có người dùng</td>
        </tr>
        @endif  
    </tbody>
